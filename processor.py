...
def process_performance(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # --- CORRECCIÓN DE ERROR: FORZAR NUMÉRICOS ---
    # Estas son las columnas que luego se promedian (mean).
    # Si alguna tiene texto (ej: "N/A"), fallará el groupby.
    cols_to_force_numeric = ["CSAT", "NPS Score", "Firt (h)", "firt_pct", "Furt (h)", "furt_pct"]
    
    for col in cols_to_force_numeric:
        if col in df.columns:
            # Limpieza extra de caracteres comunes
            df[col] = (
                df[col].astype(str)
                .str.replace("%", "", regex=False)
                .str.replace(",", ".", regex=False)
            ...
            # Convertir a número, lo que no sea número se vuelve NaN
    ...
    # Encuestas (si CSAT o NPS existen)
...
def process_auditorias(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # --- CORRECCIÓN: FORZAR NUMÉRICO EN NOTA ---
    score_raw = (
...
def process_inspecciones(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Cálculos seguros con fillna(0) si se desea o manejando nans
    ...
    cols_agg = {
        "Inspecciones_Q": "sum",
        "Cump_Exterior": "sum", "Incump_Exterior": "sum",
        "Cump_Interior": "sum", "Incump_Interior": "sum",
        "Cump_Conductor": "sum", "Incump_Conductor": "sum",
    }
    
    diario = df.groupby("fecha", as_index=False).agg(cols_agg)
    return diario
...
def procesar_global(
    ...
    # Se recomienda capturar errores en la app, pero aquí ejecutamos directo
    ...
    # Promedios: NO convertir 0 en NaN si es Nota_Auditorias, pero el resto sí
    for c in mean_cols:
